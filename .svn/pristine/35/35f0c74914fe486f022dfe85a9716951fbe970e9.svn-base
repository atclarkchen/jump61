...
abstract class Board extends Observable {
    ...
    boolean isGameOver() {
        int size = size() * size();
        if ((numOfSide(RED) == size) || (numOfSide(BLUE) == size)) {
            return true;
        }
    ...
    final Side getWinner() {
        if ((this.numOfSide(RED) == size() * size())) {
            return RED;
        }
        if ((this.numOfSide(BLUE) == size() * size())) {
            return BLUE;
        }
        return null;
    }
    ...
    /** returns Stack of SquareX objects. */
